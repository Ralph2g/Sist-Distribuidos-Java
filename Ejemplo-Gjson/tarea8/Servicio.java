...
*/
/*
  Modificación Uriel Villegas 8/12/2020
*/
...
public class Servicio
{
  ...
  //Función que borra todos los usuarios y sus imagenes
  @POST
  @Path("limpiar")
  @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
  @Produces(MediaType.APPLICATION_JSON)
  public Response borra_todos() throws Exception
  {
    ...
    try
    {
      PreparedStatement stmt_1 = conexion.prepareStatement("SELECT 1 FROM usuarios");
      try
      {
        ResultSet rs = stmt_1.executeQuery();
        try
        {
          if (!rs.next())
		      return Response.status(400).entity(j.toJson(new Error("Pos no pude"))).build();
        }
        finally
        {
          rs.close();
